...
# Install whichever LLM CLI(s) you want to use:
npm install -g @anthropic-ai/claude-code   # claude  (--provider claude)
# or: install kimi CLI                     # kimi    (--provider kimi)
# or: install gemini CLI                   # gemini  (--provider gemini)
...
```bash
# Single-branch feature (default provider: claude)
./workflow/orchestrate.sh specs/my-feature.md

# Use Kimi as the LLM
./workflow/orchestrate.sh specs/my-feature.md --provider kimi

# Use Gemini as the LLM
./workflow/orchestrate.sh specs/my-feature.md --provider gemini

# Stacked PRs (large feature split into layers)
./workflow/orchestrate.sh specs/my-feature.md --stack --provider kimi
...
> `CLAUDE.md` is the standard name used by the context generator — it works regardless of which LLM provider you run the workflow with.
...
| `--provider PROVIDER` | claude | LLM CLI to use: `claude`, `kimi`, or `gemini` |
| `--model MODEL` | claude-opus-4-6 | Model name — only used when `--provider claude` |

---

## Tips

- **Complex codebases**: add a detailed `CLAUDE.md` (or equivalent context file) — it's included in every agent's context
- **Flaky tests**: increase `--max-retries` and ensure your test suite is deterministic before running the workflow
- **Provider selection**: use `--provider kimi` or `--provider gemini` to swap the LLM without changing anything else
- **Claude speed vs quality**: use `--model claude-sonnet-4-6` for faster/cheaper Claude runs; `--model claude-opus-4-6` for harder problems
- **Multiple reviewers**: run the same spec twice with different providers (`--provider kimi` vs `--provider gemini`) for a cross-model review
